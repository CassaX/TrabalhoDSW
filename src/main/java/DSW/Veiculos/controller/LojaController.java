...
public class LojaController {
    ...
    public String cadastrar(Loja loja, ModelMap model) {
        ...
        model.addAttribute("org.springframework.validation.BindingResult.loja",
                new BeanPropertyBindingResult(loja, "loja")); //
    ...
    @PostMapping("/editar")
    public String editar(@Valid Loja loja, String novaSenha, BindingResult result, RedirectAttributes attr,
            ModelMap model) {
        if (result.hasErrors()) {
            model.addAttribute("loja", loja);
            model.addAttribute("org.springframework.validation.BindingResult.loja", result); //
        ...
        try {
            ...
            if (e.getMessage().contains("Email Já em Uso")) {
                result.rejectValue("email", null, e.getMessage());
            } else if (e.getMessage().contains("CNPJ não pode ser alterado")) { // Mensagem específica para CNPJ não
                                                                                // poder ser alterado
                result.rejectValue("CNPJ", null, e.getMessage());
            } else {
    ...
    public String perfil(ModelMap model, RedirectAttributes attr) {
        ...
        if (loja == null || !"LOJA".equals(loja.getRole())) { // Salvaguarda para garantir que é uma loja
            System.err.println("Erro: Usuário logado como " + email
                    + " tentou acessar perfil de loja, mas não foi encontrado ou não tem ROLE_LOJA.");
            attr.addFlashAttribute("fail",
                    "Seu perfil de loja não foi encontrado ou você não tem permissão para acessá-lo.");
